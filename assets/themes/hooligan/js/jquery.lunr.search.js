(function($) {
  ...
  var LunrSearch = (function() {
    function LunrSearch(elem, options) {
      this.$elem = elem;      
      this.$regularContent = $(options.regularContent),
    ...
    LunrSearch.prototype.displayResults = function(entries) {
      ...
      if (entries.length === 0) {
        $entries.append("<p>Rien n'a été trouvé !</p>")
      } else {
        some_data = Mustache.to_html(this.template.text(),{entries: entries})
        $entries.append(some_data);
      }
      $regularContent.hide();
      $results.show();
